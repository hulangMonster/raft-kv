...
> （M1–M5 全部实现并发布；M5 唯一未达标项是 p=8/64 的同机比值吞吐，已定量定位，见
> [性能与已知取舍](#性能与已知取舍)；路线图见 [docs/roadmap.md](docs/roadmap.md)）
...
**(3) p=8/64 差距的定量定位**（`docs/m5-bench.md` §3.9/§3.10）：单批内部 = 锁外 fsync **8 ms** +
等 follower ack **13 ms** + 发送 **14 ms** + 间隙 3 ms ≈ 38 ms/批；整轮里 **fsync 只占墙钟 ~10%**。
并行扇出（每 peer 一条发送线程）实测**更差**（p=64 252→138 qps，已回退）；
蓄批（`--group-linger-us`）在静载机器上仍无增益（241/244/235 qps）。
⇒ 瓶颈在**批与批之间的唤醒 + 单把全局锁争用**；M4 更快是因为它把 fsync 关在锁里形成全局串行点，
这是 **I9（锁内不做 IO）与高并发吞吐的结构性张力**，下一步要改唤醒机制（按批条件变量/事件计数），
属独立设计任务。
...
3. **先量化再优化、负结果也入档**：绝对性能判据先被独立微基准证伪（8 ms 提交下限），再改成同机比值口径；
   两次失败的优化尝试（并行扇出、蓄批）都带数据写进 `docs/m5-bench.md`，避免后人重走。
